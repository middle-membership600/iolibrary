...
struct io61_file
{
    ...
    const static int block_sz = 128;

    std::mutex m;
    std::condition_variable_any cv;
    std::map<off_t, off_t> file_locks; // map of all locked file ranges.
};

bool may_overlap_with_other_lock(io61_file *f, off_t start, off_t len)
{
    return (f->file_locks.lower_bound(start) != f->file_locks.lower_bound(start + len)); // check for overlap
}
...
{
    std::unique_lock guard(f->m); // coarse grain lock
    assert(!f->positioned);
...
int io61_flush_helper(io61_file *f) // flush helper to prevent deadlock
{
...
int io61_flush(io61_file *f) // user flush
{
    std::unique_lock guard(f->m); // coarse grain lock
...
int io61_try_lock(io61_file *f, off_t start, off_t len, int locktype)
{
    ...
    f->file_locks.insert({start, start + len});
    return 0;
}
